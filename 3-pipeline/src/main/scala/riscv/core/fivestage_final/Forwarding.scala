...
class Forwarding extends Module {
  ...
  when( io.rd_mem === io.rs1_ex && io.reg_write_enable_mem && io.rd_mem =/= 0.U) {
    // Condition 1: MEM stage wriWtes register
  ...
  when(io.rs2_ex === io.rd_mem && io.reg_write_enable_mem && io.rd_mem =/= 0.U) {
  ...
  }.elsewhen(io.rs2_ex === io.rd_wb && io.reg_write_enable_wb && io.rd_wb =/= 0.U) {
  ...
  when(io.rs1_id === io.rd_mem && io.reg_write_enable_mem && io.rd_mem =/= 0.U) {
  ...
  }.elsewhen(io.rs1_id === io.rd_wb && io.reg_write_enable_wb && io.rd_wb =/= 0.U) {
    // Condition 1: WB stage writes register
    // Condition 2: Register address match
    // Condition 3: Not x0 register
    //
  ...
  when(io.rs2_id === io.rd_mem && io.reg_write_enable_mem && io.rd_mem =/= 0.U) {
  ...
  }.elsewhen(io.rs2_id === io.rd_wb && io.reg_write_enable_wb && io.rd_wb =/= 0.U) {
